...
class TestNode():
    ...
    def __init__(self, i, dirname, extra_args, rpchost, timewait, binary, stderr, mocktime, coverage_dir):
        ...
        if timewait:
            ...
            self.rpc_timeout = 60
        if binary is None:
            self.binary = os.getenv("AVIAND", "aviand")
        else:
            self.binary = binary
        self.stderr = stderr
...
>>>>>>> 5c221fd42c641eee3fbeae37afdd33a8d39efec4
    ...
    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the aviand process. Returns False if unable to connect."""
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcassword. aviand still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        raise AssertionError("Unable to connect to aviand")
    ...
    def node_encrypt_wallet(self, passphrase):
        ...
        This causes aviand to shutdown, so this method takes
...
class TestNodeCLI():
    ...
    def send_cli(self, command, *args, **kwargs):
        """Run avian-cli command. Deserializes returned string as python object."""

        pos_args = [str(arg) for arg in args]
        named_args = [str(key) + "=" + str(value) for (key, value) in kwargs.items()]
        assert not (pos_args and named_args), "Cannot use positional arguments and named arguments in the same avian-cli call"
